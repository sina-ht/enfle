...
"rowloop:\n\t"
	...
	"psrad    $11,  %%mm3\n\t"           //         ; y6=a1-b1 y7=a0-b0 (11 == SHIFT_INV_ROW)
	"paddd  %%mm4,  %%mm1\n\t"           // 4 	; a1+b1 a0+b0

	"paddd  %%mm2,  %%mm0\n\t"           // 2 	; a3=sum(even3) a2=sum(even2)
	"psrad    $11,  %%mm1\n\t"           //         ; y1=a1+b1 y0=a0+b0 (11 == SHIFT_INV_ROW)
	...
	"psrad   $11, %%mm0\n\t"             //         ; y3=a3+b3 y2=a2+b2 (11 == SHIFT_INV_ROW)
	"psrad   $11, %%mm4\n\t"             //         ; y4=a3-b3 y5=a2-b2 (11 == SHIFT_INV_ROW)
	...
	"add    $16, %%eax\n\t"              // add 1 row to input pointer
	"add     $8, %%ebx\n\t"              // go to next rounding values
	"add    $16, %%edx\n\t"              // add 1 row to output pointer
	"add    $64, %%ecx\n\t"              // move to next section of table
...
// slot
	...
	"psraw	   $6, %%mm3\n\t"      // dst1 (6 == SHIFT_INV_COL)
	"psubsw	%%mm1, %%mm7\n\t"      // a1-b1

	"psraw	   $6, %%mm6\n\t"      // dst2 (6 == SHIFT_INV_COL)
	"psubsw	%%mm4, %%mm0\n\t"      // a2-b2

//	"movq	[SCRATCH+0], %%mm1\n\t" // load b0
	"movq	48(%%edx), %%mm1\n\t"  // load b0
	"psraw	   $6, %%mm7\n\t"      // dst6 (6 == SHIFT_INV_COL)

	"movq	%%mm5, %%mm4\n\t"      // a0
	"psraw	   $6, %%mm0\n\t"      // dst5 (6 == SHIFT_INV_COL)
...
//	"movq	[SCRATCH+8], %%mm3     // load b3
	"movq	80(%%edx), %%mm3\n\t"  // load b3
	"psraw	       $6, %%mm5\n\t"  // dst0 (6 == SHIFT_INV_COL)

	"movq	%%mm2, %%mm6\n\t"      // a3
	"psraw	       $6, %%mm4\n\t"  // dst7 (6 == SHIFT_INV_COL)
	...
	"movq	%%mm5, (%%edx)\n\t"
	"psraw	   $6, %%mm2\n\t"      // dst3 (6 == SHIFT_INV_COL)

	"movq	%%mm4, 112(%%edx)\n\t"
	"psraw	   $6, %%mm6\n\t" // dst4 (6 == SHIFT_INV_COL)
	...
	"mov %%esi, %%eax\n\t"
	"mov %%ebp, %%edx\n\t"
	"add $8, %%eax\n\t"            // add 4 cols to input pointer
	"add $8, %%edx\n\t"            // add 4 cols to output pointer
