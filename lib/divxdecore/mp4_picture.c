...
#if 0
static unsigned int post_hor_count = 0;
static unsigned int post_ver_count = 0;
static unsigned int post_dering_count = 0;

static unsigned int post_hor_th = TH_V;
static unsigned int post_ver_th = TH_V;
static unsigned int post_dering_th = TH_V;
#endif
...
{ 
	/***
	int post_hor_flag = post_hor_count >= post_hor_th; // after N consecutive render the hor PP is activated
	int post_ver_flag = post_ver_count >= post_ver_th; // after N consec. hor PP the ver PP is also activated
	int post_dering_flag = post_dering_count >= post_dering_th;
	***/
	...
	if (render_flag) 
	{
		if (post_flag)
		{
			...
			convert_yuv(display_frame[0], mp4_hdr.width,
				...
				bmp, mp4_hdr.width, mp4_hdr.height);
		}
		else
		{
...
/***
		if (post_dering_flag) {
			postprocess(frame_ref, coded_picture_width,
				display_frame, horizontal_size, 
				horizontal_size,  vertical_size, 
				&quant_store[1][1], (MBC+1), PP_DEBLOCK_Y_H | PP_DEBLOCK_Y_V | PP_DERING_Y);
			convert_yuv(display_frame[0], mp4_hdr.width,
				display_frame[1], display_frame[2], (mp4_hdr.width>>1),
				bmp, mp4_hdr.width, mp4_hdr.height);
		}
		else if (post_ver_flag) {
			postprocess(frame_ref, coded_picture_width,
				display_frame, horizontal_size, 
				horizontal_size,  vertical_size, 
				&quant_store[1][1], (MBC+1), PP_DEBLOCK_Y_H | PP_DEBLOCK_Y_V);
			convert_yuv(display_frame[0], mp4_hdr.width,
				display_frame[1], display_frame[2], (mp4_hdr.width>>1),
				bmp, mp4_hdr.width, mp4_hdr.height);
			post_dering_count++;
		}
		else if (post_hor_flag) {
			postprocess(frame_ref, coded_picture_width,
				display_frame, horizontal_size, 
				horizontal_size,  vertical_size, 
				&quant_store[1][1], (MBC+1), PP_DEBLOCK_Y_H);
			convert_yuv(display_frame[0], mp4_hdr.width,
				display_frame[1], display_frame[2], (mp4_hdr.width>>1),
				bmp, mp4_hdr.width, mp4_hdr.height);
			post_ver_count++;
	...
	else // can't render, too much slow
	{
		// check what level of PP was activated
		if (post_hor_count > post_hor_th) 
		{
			if (post_ver_count > post_ver_th) 
			{
				if (post_dering_count > post_dering_th) 
				{
					// dering
					post_dering_count = 0;
					post_dering_th += TH_V;
