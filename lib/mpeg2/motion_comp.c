...
void mpeg2_mc_init (uint32_t mm_accel_flag)
{
#ifdef __i386__
    if (mm_accel_flag & MM_ACCEL_X86_MMXEXT) {
	fprintf (stderr, "Using MMXEXT for motion compensation\n");
	mpeg2_mc = mpeg2_mc_mmxext;
    } else if (mm_accel_flag & MM_ACCEL_X86_3DNOW) {
	fprintf (stderr, "Using 3DNOW for motion compensation\n");
	mpeg2_mc = mpeg2_mc_3dnow;
    } else if (mm_accel_flag & MM_ACCEL_X86_MMX) {
	fprintf (stderr, "Using MMX for motion compensation\n");
	mpeg2_mc = mpeg2_mc_mmx;
    } else
#endif
#ifdef __ppc__
    if (mm_accel_flag & MM_ACCEL_PPC_ALTIVEC) {
	fprintf (stderr, "Using altivec for motion compensation\n");
	mpeg2_mc = mpeg2_mc_altivec;
    } else
#endif
#ifdef LIBMPEG2_MLIB
    if (mm_accel_flag & MM_ACCEL_MLIB) {
