 ...
 * Last Modified: Tue Sep 18 13:42:00 2001.
 * $Id: vmpm_decompose_recur3.c,v 1.2 2001/09/18 05:22:24 sian Exp $
 */
...
static VMPMDecomposer plugin = {
  name: "Recur3",
  description: "Recursive MPM decomposer 3",
  init: init,
  decompose: decompose,
  encode: encode,
  decode: decode,
...
static int
calc_lowest_level(VMPM *vmpm, Arithmodel **ams, int pos)
{
  unsigned int i, max, ndtokens, rpow = vmpm->r;
...
static int
calc_highest_level(VMPM *vmpm, Arithmodel *level_am, int pos)
{
  unsigned int i, r;

  for (i = 1, r = vmpm->r; i < arithmodel_order_zero_nsymbols(level_am); i++, r *= vmpm->r)
    if (pos % r)
      return i - 1;
  return i - 1;
}
